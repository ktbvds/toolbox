...
if [ -e "/etc/pki/tls/certs/server.pem" ];then
    ...
    umask 77
    openssl req -utf8 -newkey rsa:2048 -keyout /etc/pki/tls/certs/server.key -nodes -x509 -days 1825 -out /etc/pki/tls/certs/server.crt -set_serial 0 -subj "/C=CN/ST=Beijing/L=Beijing/O=Company Inc/CN=$HOSTNAME"
